...
# --- Función para checkboxes con "Seleccionar/Deseleccionar todo" ---
def checkbox_con_todo(label, opciones, key):
    st.sidebar.markdown(f"**{label}**")

    # Inicializar estado del "Seleccionar todo"
    if f"{key}_todo" not in st.session_state:
        st.session_state[f"{key}_todo"] = True

    seleccionar_todo = st.sidebar.checkbox(
        f"Seleccionar/Deseleccionar todo {label}",
        value=st.session_state[f"{key}_todo"],
        key=f"{key}_checkbox_todo"
    )
    st.session_state[f"{key}_todo"] = seleccionar_todo

    seleccionadas = []
    for o in sorted(opciones):
        # Cada checkbox se inicializa según el estado del "Seleccionar todo"
        if st.sidebar.checkbox(str(o), value=st.session_state[f"{key}_todo"], key=f"{key}_{o}"):
...
seleccion_hubs = checkbox_con_todo("HUB Agroecológico", hubs, "hub")
...
seleccion_categorias = checkbox_con_todo("Categoría del Proyecto", categorias, "categoria")
...
seleccion_proyectos = checkbox_con_todo("Proyecto", proyectos, "proyecto")
...
seleccion_ciclos = checkbox_con_todo("Ciclo", ciclos, "ciclo")
...
seleccion_tipos_parcela = checkbox_con_todo("Tipo de Parcela", tipos_parcela, "parcela")
...
seleccion_estados = checkbox_con_todo("Estado", estados, "estado")
...
seleccion_anio = checkbox_con_todo("Año", opciones_anio, "anio")
...
seleccion_sistema = checkbox_con_todo("Tipo de sistema", opciones_sistema, "sistema")
...
seleccion_cultivos = checkbox_con_todo("Cultivo Principal", opciones_cultivo, "cultivo")
